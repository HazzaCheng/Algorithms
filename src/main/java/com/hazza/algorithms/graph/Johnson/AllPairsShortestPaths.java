...
public class AllPairsShortestPaths {
    ...
    public static int[][] johnson(int m, Edge[] edges) {
        ...
        if (com.hazza.algorithms.graph.BellmanFord.FindShortestPath.bellmanFord(m + 1, m + 1, newEdges)) {
            for (int i = 1; i <= m; i++)
                h[i] = com.hazza.algorithms.graph.BellmanFord.FindShortestPath.getDistanceFromSource(i);
        ...
        for (int i = 1; i <= m; i++) {
            com.hazza.algorithms.graph.Dijkstra.FindShortestPath.dijkstra(m, i, edges);
            for (int j = 1; j <= m; j++)
                matrix[i][j] = h[j] - h[i]
                        + com.hazza.algorithms.graph.Dijkstra.FindShortestPath.getDistanceFromSource(j);
